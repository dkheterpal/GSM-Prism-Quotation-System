...
@section('content')
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8 animate-fade-in"
        ...
        <div class="flex flex-col sm:flex-row gap-4 mt-6 sm:mt-0 w-full md:w-auto">
            <form action="{{ route('quotes.index') }}" method="GET" class="relative w-full sm:w-auto">
                <input type="hidden" name="status" value="{{ $currentStatus }}">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search quotes..."
                    class="w-full sm:w-64 pl-10 pr-20 py-2.5 sm:py-2 border border-slate-200 rounded-lg shadow-sm focus:ring-brand-500 focus:border-brand-500 bg-white text-sm transition-all focus:w-full sm:focus:w-80">
