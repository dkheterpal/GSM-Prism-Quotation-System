...
@section('content')
    <div class="max-w-4xl mx-auto flex flex-col items-stretch space-y-6">
        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data"
            class="bg-white rounded-2xl shadow-xl shadow-slate-200/50 overflow-visible border border-slate-100 flex flex-col space-y-0 w-full"
            ...
            <input type="hidden" name="prices_to_delete" id="prices_to_delete" value="">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                    ...
                    <div class="md:col-span-2">
                        ...
                        @if($product->image_path)
                            <div class="mt-4 flex items-center gap-4">
                                <img src="{{ url($product->image_path) }}" class="h-20 border rounded object-contain">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="4"
                            class="w-full rounded-lg border-slate-200 shadow-sm focus:border-slate-500 focus:ring-slate-500 py-2.5 px-4 bg-white wysiwyg">{{ old('description', $product->description) }}</textarea>
                    </div>
                    <div class="md:col-span-2">
                        ...
                        <textarea name="deliverables" rows="4"
                            class="w-full rounded-lg border-slate-200 shadow-sm focus:border-slate-500 focus:ring-slate-500 py-2.5 px-4 bg-white wysiwyg">{{ old('deliverables', $product->deliverables) }}</textarea>
                    </div>
                    <div class="md:col-span-2">
                        ...
                        <textarea name="project_timeline" rows="4"
                            class="w-full rounded-lg border-slate-200 shadow-sm focus:border-slate-500 focus:ring-slate-500 py-2.5 px-4 bg-white wysiwyg">{{ old('project_timeline', $product->project_timeline) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="p-8">
                ...
                <div id="prices-container" class="space-y-4 w-full">
        ...
        <div
            ...
            <div>
                ...
                <input type="number" step="0.01" min="0" required name="prices[__INDEX__][setup_fee]" value="0"
                    class="w-full bg-slate-50 border border-slate-200 rounded p-2 text-sm price-setup-input">
            </div>
            <div class="monthly-wrapper">
                ...
                <input type="number" step="0.01" min="0" required name="prices[__INDEX__][monthly_fee]" value="0"
                    class="w-full bg-slate-50 border border-slate-200 rounded p-2 text-sm price-monthly-input">
            </div>
            <div class="term-wrapper">
                ...
                <input type="number" min="0" name="prices[__INDEX__][term_months]"
                    class="w-full bg-slate-50 border border-slate-200 rounded p-2 text-sm price-term-input">
            </div>
    ...
    @push('scripts')
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const pricesToDeleteInput = document.getElementById('prices_to_delete');
                let deletedPrices = [];

                function addPriceRow(priceId = '', type = 'Fixed', setup = '0', monthly = '0', term = '') {
                    const clone = template.content.cloneNode(true);
                    const row = clone.querySelector('.price-row');

                    row.querySelectorAll('input:not(.price-setup-input):not(.price-monthly-input):not(.price-term-input), select').forEach(el => {
                        el.name = el.name.replace('__INDEX__', priceIndex);
                    });

                    row.querySelector('input[name="prices[' + priceIndex + '][id]"]').value = priceId;

                    row.querySelector('.price-setup-input').name = `prices[${priceIndex}][setup_fee]`;
                    row.querySelector('.price-setup-input').value = setup;

                    row.querySelector('.price-monthly-input').name = `prices[${priceIndex}][monthly_fee]`;
                    row.querySelector('.price-monthly-input').value = monthly;

                    row.querySelector('.price-term-input').name = `prices[${priceIndex}][term_months]`;
                    row.querySelector('.price-term-input').value = term;

                    row.querySelector('.price-type-select').value = type;
                    ...
                    function toggleFields() {
                        ...
                        if (isSubscription) {
                            ...
                            row.querySelector('.price-monthly-input').value = '0';
                            row.querySelector('.price-term-input').value = '';
                        }
                    }
                    ...
                    row.querySelector('.remove-price').addEventListener('click', () => {
                        if (priceId) {
                            deletedPrices.push(priceId);
                            pricesToDeleteInput.value = deletedPrices.join(',');
                        }
                        row.remove();
                    });
                ...
                const initialPrices = @json($product->prices);
                if (initialPrices && initialPrices.length > 0) {
                    initialPrices.forEach(p => {
                        addPriceRow(p.id, p.type, p.setup_fee, p.monthly_fee, p.term_months || '');
                    });
                } else {
                    addPriceRow();
                }
